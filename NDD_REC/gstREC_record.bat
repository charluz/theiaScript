...
@echo on
adb shell gst-launch-1.0 -v v4l2src device="/dev/video3" num-buffers=%recFrames% ! video/x-raw,format=\(string\)YUY2,width=%fstOrderSizeW%,height=%fstOrderSizeH%, framerate=30/1 ! mtkmdp width=%sizeW% height=%sizeH% ! video/x-raw,format=I420 ! v4l2mtkh264enc bitrate=9000000 gop=8 ! avimux ! filesink location=%camDir%/%aviName%.avi
@echo off
